...
'#Region "Actions"
        Public SimpleFormulasAction As Action(Of Stream, XlDocumentFormat) = AddressOf SimpleFormula

        Public ComplexFormulasAction As Action(Of Stream, XlDocumentFormat) = AddressOf ComplexFormulas

        Public SharedFormulasAction As Action(Of Stream, XlDocumentFormat) = AddressOf SharedFormulas

        Public SubtotalsAction As Action(Of Stream, XlDocumentFormat) = AddressOf Subtotals

'#End Region
        Private Sub SimpleFormula(ByVal stream As Stream, ByVal documentFormat As XlDocumentFormat)
'#Region "#SimpleFormula"
            ...
            Using document As IXlDocument = exporter.CreateDocument(stream)
                ...
                Using sheet As IXlSheet = document.CreateSheet()
                    ...
                    Dim header As String() = New String() {"Description", "QTY", "Price", "Amount"}
                    Dim product As String() = New String() {"Camembert", "Gorgonzola", "Mascarpone", "Mozzarella"}
                    Dim qty As Integer() = New Integer() {12, 15, 25, 10}
                    Dim price As Double() = New Double() {23.25, 15.50, 12.99, 8.95}
...
'#End Region  ' #SimpleFormula
        ...
        Private Sub ComplexFormulas(ByVal stream As Stream, ByVal documentFormat As XlDocumentFormat)
            ' Create an exporter instance.
            Dim exporter As IXlExporter = XlExport.CreateExporter(documentFormat, New XlFormulaParser())
            ' Create a new document.
            Using document As IXlDocument = exporter.CreateDocument(stream)
                ...
                Using sheet As IXlSheet = document.CreateSheet()
                    ...
                    ' Generate data for the document.
                    Dim header As String() = New String() {"Description", "QTY", "Price", "Amount"}
                    Dim product As String() = New String() {"Camembert Pierrot", "Gorgonzola Telino", "Mascarpone Fabioli", "Mozzarella di Giovanni"}
                    Dim qty As Integer() = New Integer() {12, 15, 25, 10}
                    Dim price As Double() = New Double() {23.25, 15.50, 12.99, 8.95}
                    ' Create the header row.
                    Using row As IXlRow = sheet.CreateRow()
...
'#Region "#Formula_XlExpression"
                    ...
                    Using row As IXlRow = sheet.CreateRow()
                        ...
                        Using cell As IXlCell = row.CreateCell()
                            ...
                            ' =$D$6/4
                            Dim expression As XlExpression = New XlExpression()
...
'#End Region  ' #Formula_XlExpression
        ...
        Private Sub SharedFormulas(ByVal stream As Stream, ByVal documentFormat As XlDocumentFormat)
            ' Create an exporter instance.
            Dim exporter As IXlExporter = XlExport.CreateExporter(documentFormat, New XlFormulaParser())
            ' Create a new document.
            Using document As IXlDocument = exporter.CreateDocument(stream)
...
'#End Region  ' #SharedFormulas
        ...
        Private Sub Subtotals(ByVal stream As Stream, ByVal documentFormat As XlDocumentFormat)
            ...
            Using document As IXlDocument = exporter.CreateDocument(stream)
                ...
                Using sheet As IXlSheet = document.CreateSheet()
                    ...
                    Dim productsDairy As String() = New String() {"Camembert Pierrot", "Gorgonzola Telino", "Mascarpone Fabioli", "Mozzarella di Giovanni"}
                    Dim productsCereals As String() = New String() {"Gnocchi di nonna Alice", "Gustaf's Knäckebröd", "Ravioli Angelo", "Singaporean Hokkien Fried Mee"}
                    ' Create the header row.
                    Using row As IXlRow = sheet.CreateRow()
